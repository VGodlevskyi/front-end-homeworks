...
const menuItem = document.querySelector(".navbar-wrap");

menu.addEventListener("click", () => {
	menuItem.classList.toggle("navbar-wrap--active");
});
...
const clickTab = function (tabWrap, tabItem, contentItem) {
	tabWrap.addEventListener("click", (event) => {
		const tabActive = document.querySelector(
			`[data-servises-content ='${event.target.dataset.servisesTitle}']`
		);
		contentItem.forEach((element) =>
			element.classList.remove("services-card-content-item-active")
		);
		tabItem.forEach((element) =>
			element.classList.remove("card-name-menu-active")
		);
...
const addItem = function (wrap, item, imgs) {
	const figure = imgs.map((imgSrc) => {
		...
		if (
			imgSrc.split("/")[1] === "graphic design" ||
			imgSrc.split("/")[1] === "web design"
		) {
			hidI.querySelector(".title-work-img").textContent = "creative design";
		} else if (
			imgSrc.split("/")[1] === "landing page" ||
			imgSrc.split("/")[1] === "wordpress"
		) {
...
const changeItem = function (galleryItem) {
	tabWrapWork.addEventListener("click", (event) => {
		tabItemWork.forEach((element) =>
			element.classList.remove("work-galery-menu-item-acrive")
		);
